import { Currency, ISOCurrencyCode, ISOCurrencies } from '../Currency'
import { IMoney as Money, AbstractMoney } from './'
import {
  isFloat,
...
export default class ISOMoney
  extends AbstractMoney<ISOCurrencyCode>
  implements Money<ISOCurrencyCode>
{
  /**
   ...
   * ```
   */
  constructor(currency: ISOCurrencyCode, amount: number) {
    if (false === Number.isInteger(amount) && isFloat(amount)) {
      throw new TypeError(
        `\`amount\` '${amount} cannot be a float (hint: use ` +
          `\`Money.fromFloat()\` instead)`,
      )
    }
    if ('string' === typeof amount) {
      throw new TypeError(
        `\`amount\` '${amount}' cannot be a string (hint: use ` +
          `\`Money.fromString()\` instead)`,
      )
    }
    assertSupportedISOCurrency(currency)
   ...
   */
  static fromFloat(currency: ISOCurrencyCode, amount: number): ISOMoney {
    if (true === Number.isInteger(amount)) {
      throw new TypeError(
        `\`amount\` '${amount}' cannot be an integer (hint: construct ` +
          `Money directly using \`new Money()\` instead)`,
      )
    }
    if (false === isFloat(amount)) {
   ...
   * @category Static Constructor
   */
  static fromString(currency: ISOCurrencyCode, amount: string): ISOMoney {
   ...
   * ```
   */
  public override getCurrency(): ISOCurrencyCode {
   ...
   * ```
   */
  add(...addends: ISOMoney[]): ISOMoney {
    assertValidMonies(this, ...addends)

    const sum = addends.reduce(
      (acc, curr) => acc + curr.getAmount(),
      this.getAmount(),
    )
   ...
   * ```
   */
  subtract(...subtrahends: ISOMoney[]): ISOMoney {
    assertValidMonies(this, ...subtrahends)

    const difference = subtrahends.reduce(
      (acc, curr) => acc - curr.getAmount(),
      this.getAmount(),
    )
   ...
   * ```
   */
  multiply(multiplier: number): ISOMoney {
   ...
   * ```
   */
  divide(divisor: number): ISOMoney {
    // @TODO This should support a custom rounding method
   ...
   * ```
   */
  equals(other: ISOMoney): boolean {
    assertAllMoney(other)
    return (
   ...
   * ```
   */
  greaterThan(other: ISOMoney): boolean {
    assertValidMonies(this, other)
   ...
   * ```
   */
  greaterThanOrEqual(other: ISOMoney): boolean {
    assertValidMonies(this, other)
    return this.getAmount() >= other.getAmount()
  }
   ...
   * ```
   */
  lessThan(other: ISOMoney): boolean {
    assertValidMonies(this, other)
    return this.getAmount() < other.getAmount()
  }
   ...
   * ```
   */
  lessThanOrEqual(other: ISOMoney): boolean {
    assertValidMonies(this, other)
    return this.getAmount() <= other.getAmount()
  }
   ...
   * ```
   */
  toJSON(): { currency: ISOCurrencyCode; amount: number } {
  ...
  ): Currency<ISOCurrencyCode>['units'] {
    return ISOCurrencies[currency]?.units
  }
}
